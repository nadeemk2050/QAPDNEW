...
async function run() {
  ...
  if (registry.documents && registry.documents.length > 0) {
    ...
    const url = `https://firestore.googleapis.com/v1/projects/cashshams/databases/(default)/documents/companies_live/${companyId}/records?pageSize=100`;
    ...
    console.log("No registry documents found or permission error:", registry);
  }
}
